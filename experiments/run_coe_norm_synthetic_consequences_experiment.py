...
from utils import (compute_cls_metrics, compute_gen_metrics, convert_examples_to_features, MoralStoriesProcessor, 
                   set_seed)
...
TASKS = ['consequence|action+context_gen', 'consequence+action+context_cls', 'norm|actions+context+consequences_gen']
...
def norm_gen_with_csq_pred(args, split):
    """ Predicts norms after hallucinating consequences for each action. """

    specified_batch_size = args.per_gpu_eval_batch_size
    args.p = 0.90

    # Generate consequences using the consequence|story+action generator
    args.consequence_generation_model_type = args.consequence_generation_model_type.lower()
    config_class, model_class, tokenizer_class = MODEL_CLASSES[args.consequence_generation_model_type]
    ...
    eval_dataset = load_and_cache_examples(args, tokenizer, split, 'consequence|action+context_gen',
                                           args.consequence_generation_model_type)
    eval_sampler = SequentialSampler(eval_dataset)
    if args.consequence_generation_model_type == 'gpt2':
    ...
    logging.info('\n' + '*' * 20)
    logging.info('Generating consequences using the model from checkpoint {}'
                 .format(args.consequence_generator_checkpoint))
    logging.info('*' * 20 + '\n')

    consequence_predictions, _, _ = test_gen(args,
                                             model,
                                             tokenizer,
                                             eval_dataset,
                                             eval_dataloader,
    ...
    consequence_pass_generations = {pass_id: list() for pass_id in range(args.num_actions)}
    ...
    # Include model generations into the input data for the norm|story+consequence generator
    args.norm_generation_model_type = args.norm_generation_model_type.lower()
    config_class, model_class, tokenizer_class = MODEL_CLASSES[args.norm_generation_model_type]
    global_step = int(args.norm_generator_checkpoint.split('-')[-1])
    model = model_class.from_pretrained(args.norm_generator_checkpoint)
    tokenizer = tokenizer_class.from_pretrained(args.norm_generator_checkpoint)
    model.to(args.device)

    # Set up data-serving pipeline
    eval_dataset = load_and_cache_examples(args, tokenizer, split, 'norm|actions+context+consequences_gen',
                                           args.norm_generation_model_type,
    ...
    # Predict norms
    logging.info('\n' + '*' * 20)
    logging.info('Generating norms using the model from checkpoint {}'
                 .format(args.norm_generator_checkpoint))
    logging.info('*' * 20 + '\n')

    norm_predictions, _, _ = test_gen(args,
                                      model,
                                      tokenizer,
                                      eval_dataset,
                                      eval_dataloader,
...
def load_and_cache_examples(args, tokenizer, split, task_name, model_type, predictions=None):
    ...
    if task_name.startswith('consequence'):
        # Remove negative examples
        filtered_examples = list()
        for ex in examples:
            if ex.moral_action is not None and ex.immoral_consequence is not None:
                continue
            elif ex.immoral_action is not None and ex.moral_consequence is not None:
                continue
            else:
        ...
        for predictions in all_predictions:
            if predictions[0] == 'actions':
                for pr_id, pr in enumerate(predictions[1]):
                    ex = examples[pr_id]
                    if ex.moral_action is not None:
                        ex.moral_action = pr
                    else:
                ...
                if task_name.startswith('consequence'):
                    ...
                    for pr_id, pr in enumerate(predictions[1]):
                        if pr_id % 2 == 0:
                            ...
                            pair.append(pr)
                            paired_predictions.append(pair)
                            pair = list()
                    for pair_id, pair in enumerate(paired_predictions):
                        ex = examples[pair_id]
                        ex.moral_consequence = pair[0]
                        ex.immoral_consequence = pair[1]
                        extended_examples.append(ex)
                    examples = extended_examples
                    extended_examples = list()
...
def main():
    ...
    parser.add_argument('--consequence_generation_model_type', default=None, type=str, required=True,
                        choices=list(MODEL_CLASSES.keys()),
                        help='Model type to use for consequence prediction, selected in the list: ' +
                             ', '.join(MODEL_CLASSES.keys()))
    parser.add_argument('--norm_generation_model_type', default=None, type=str, required=True,
                        choices=list(MODEL_CLASSES.keys()),
                        help='Model type to use for norm prediction, selected in the list: ' +
                             ', '.join(MODEL_CLASSES.keys()))
    ...
    parser.add_argument('--norm_generator_checkpoint', default=None, type=str, required=True,
                        help='Path to pre-trained model used for consequence generation')
    ...
    logger.info('Generating norms with consequence prediction:')
    _ = norm_gen_with_csq_pred(args, 'test')
